...
- 컴포넌트 3분할 — `src/components/home/PosterBackdrop.tsx`(① 그리드+루프+소스 분기),
  `src/components/common/ExtrudedText.tsx`(② 입체 압출 타이포, 2026-09-09 `OutlinedText`에서
  개정 — 상위 §9.1 참고. 스플래시·로그인 재사용 가능),
...
남긴다: **Expo Go 런타임 한정 문제였고, `expo prebuild` + Dev Client 전환으로 사라졌다**
(2026-09-11). 임시로 넣었던 `src/navigation/backGuard.ts`는 **제거했다** — Dev Client에서
가드를 끈 채 재현해도 빈 화면이 나오지 않았고, 전환 중 뒤로가기가 무시되는 체감은 그대로였다.
**네이티브가 이미 하고 있던 억제를 JS로 재구현한 것**이었기 때문이다.

⚠️ **2026-09-10 시점의 요약(*"`backGuard.ts`가 해결했다"*)은 폐기됐다.** 결론이 하루 만에
뒤집힌 항목이므로, 이 문단을 읽고 가드를 되살리지 말 것 — 되살릴 조건도 §8.6에 있다.
...
| 2026-09-10 (이어서 7) | **포스터 전체 보기 — 상태바 영역이 안 어두워지는 문제 수정(갤럭시 실기기 확인).** Android에서 `Modal`이 `statusBarTranslucent` 없이는 상태바 아래부터만 그려져 검정 배경이 최상단까지 못 덮었다 — `statusBarTranslucent` 추가. 아이콘이 검정 배경에 묻히지 않도록 모달이 열려 있는 동안만 `expo-status-bar`로 `style="light"`를 얹었다(닫히면 `App.tsx`의 전역 `auto`로 자동 복귀 — `expo-status-bar`는 여러 `<StatusBar>` 인스턴스를 스택처럼 다뤄 언마운트 시 이전 설정으로 되돌아간다). `npx tsc --noEmit`·`expo export --platform android` 통과 |
| 2026-09-10 (이어서 6) | **§9.3 재확인 후 2건 조정 — 크롭 비율 1:1 → 4:5 롤백, 캐스트 여백 확대.** ① **닫기 버튼은 정상 확인.** ② **출연진 첫 아바타가 여전히 살짝 잘려** `paddingLeft`를 3→8로 늘렸다. ③ **정사각형(1:1) 크롭이 "너무 줄었다"는 피드백**으로 **4:5로 롤백** — 정보 박스 노출과 포스터 비중 사이의 정확한 지점은 사용자가 추후 직접 미세조정하기로 했다. `npx tsc --noEmit`·`expo export --platform android` 통과 |
| 2026-09-10 (이어서 5) | **§9.3 실기기 확인 후 3건 수정.** ① **포스터 전체 보기 닫기 버튼 신설** — 이미지 탭으로 닫히는 게 오히려 불편하다는 피드백. 배경을 `Pressable`에서 `View`로 바꾸고 우상단(safe area 반영)에 명시적 `X` 닫기 버튼을 뒀다. ② **출연진 첫 아바타 좌측 잘림** — 원형 아바타가 가로 `ScrollView` 경계에 딱 붙어 왼쪽 끝이 살짝 잘려 보였다. `contentContainerStyle={{ paddingLeft: 3 }}`로 여유를 줬다. ③ **기본 정보 박스가 첫 화면에서 살짝 스크롤 밖으로 남음** — 4:5(587px, 58%)로도 부족해 **정사각형(1:1, 46%)** 으로 한 번 더 줄였다. 상위 §9.3이 검토했던 옵션 중 하나("포스터 느낌이 옅어진다"는 우려가 있었던 안)를 실기기 결과에 따라 채택한 것 — 첫 화면 정보 노출을 우선했다. `npx tsc --noEmit`·`expo export --platform android` 통과 |
| 2026-09-10 (이어서 4) | **§9.3 히어로 재구현 — 4:5 상단 크롭 + 배경색 페이드 + 포스터 전체 보기.** 상위 `M2-frontend-spec.md` §9.3 재설계(2026-09-10, 사용자 작성)를 그대로 구현. 진짜 원인은 "제목을 포스터 위에 얹으면 어두운 스크림이 필요하고, 흰 배경으로 이어질 때 값이 반전돼 탁한 회색 띠가 생긴다"는 것 — 그래서 제목·년도·러닝타임을 히어로 **밖**(가운데 정렬)으로 뺐다. 히어로는 4:5 컨테이너(`overflow: hidden`)에 2:3 원본 이미지를 `top:0`으로 두어 **아래쪽만** 자르고(`resizeMode="cover"`를 쓰면 가운데 기준이라 인물·타이틀이 있는 위쪽이 잘린다), 하단 38%를 `rgba(255,255,255,0)→0.72→colors.background`로 페이드해 아래 콘텐츠와 색 경계 없이 이어지게 했다. 크롭했기 때문에 **히어로 탭 → 포스터 전체 보기**(`Modal` + `resizeMode="contain"` + 어두운 배경, `ActionSheet`와 같은 탭-배경-닫기 패턴)를 신설 — `PosterSize.HERO`(`w780`)를 그대로 재사용해 추가 다운로드 없이 즉시 뜬다. 크롭된 화면은 잘렸다는 티가 안 나 우하단에 `Maximize2` 힌트 아이콘을 뒀다. **`expo-blur` 의존 제거**(`npm uninstall expo-blur`) — 이 화면이 유일한 사용처였다. 핀치 줌은 스펙이 명시한 대로 백로그로 남겨 구현하지 않았다. `npx tsc --noEmit`·`expo export --platform android`(모듈 수 3373→3369, `expo-blur` 제거 확인) 통과. 실기기 검증 전 |
| 2026-09-10 (이어서 3) | **`OutlinedText` → `ExtrudedText` 구현 — 로고를 입체 압출 타이포로 교체.** 설계는 상위 `M2-frontend-spec.md` §9.1(2026-09-09 개정)이 단일 출처다. `src/components/common/OutlinedText.tsx`를 지우고 `ExtrudedText.tsx` 신설 — `depth`(기본 `fontSize × 0.055`, 최소 2 보장)·`direction`(기본 ↘)·`extrudeColor`·`keylineColor`를 prop으로 받는다. 그리는 순서를 스펙대로 뒤집었다 — 기존 `CORNERS` 역순 규칙(인덱스 0이 맨 위) 대신 **가장 먼 층(키라인)부터 그려 본체가 맨 위**에 오도록 `i = depth+1 → 1` 순으로 내려가며 렌더하고 본체는 마지막에 별도로 그린다. 본체를 제외한 전 층에 접근성 숨김을 유지했다. `HomeScreen.tsx`에서 `extrudeColor={colors.brandDeep}`·`keylineColor={colors.brandLight}`로 교체(새 토큰 없음, 기존 `outlineColor` 4색 배열 방식 제거). `npx tsc --noEmit` 통과. **실기기 검증 전이다** — 대각선 압출이 시안처럼 보이는지, 폰트 두께(§9.1 미결 항목)가 아쉬운지는 실기기에서 확인해야 한다 |
| 2026-09-10 (이어서 2) | **히어로 블러를 `expo-blur`(`BlurView`)로 교체.** `Image blurRadius` 5단 근사에도 실기기에서 계단이 뚜렷이 보인다는 재확인을 받고, 도입 비용(prebuild 불필요, 다만 진짜 매끄러운 블러엔 결국 마스크 라이브러리가 더 필요, Android 성능은 실기기 확인 전엔 장담 못함)을 설명한 뒤 **전환하기로 했다.** `npx expo install expo-blur`(`~57.0.2`, SDK 57 호환) 설치. `BlurView`는 자기 위치에서 바로 밑을 블러 계산해 줘서 **이미지를 잘라 겹치는 수작업이 사라졌다** — `HERO_BLUR_RADII`(Image용) 대신 `HERO_BLUR_INTENSITIES = [15,35,55,80,100]` 5단 `intensity` + `tint="dark"`로 교체, 코드도 더 짧아졌다. 홈 배경 때 `expo-blur`를 피한 이유(60초 루프 애니메이션 위에 실시간 재계산, Android 성능 우려)는 여기 적용되지 않는다고 판단했다 — `MovieDetail` 히어로는 스크롤해도 안 움직이는 정적 이미지 한 장이다. 배경색으로 빠지는 그라디언트·위쪽 65% 텍스트 배치는 그대로 유지. `npx tsc --noEmit`·`expo export --platform android`·`expo-doctor`(무관한 기존 `expo` 패치 버전 경고 1건 제외 전부 통과) 확인 |
